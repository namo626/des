...
void freeFork(Fork* fork) {
	if (fork != NULL) {
		if (fork->outports != NULL) {
			free(fork->outports);
		}
		if (fork->chances != NULL) {
			free(fork->chances);
		}
		free(fork);
	}
...
void handleArrival(Arrival* arrival) {
	...
	if (dest->type == "E") {
		...
		if (isEmpty(station) == TRUE) {
			...
			schedule(mkDeparture(leaveTime, arrival->destID), leaveTime);
		} else {
...
void handleDeparture(Departure* departure) {
	...
	if (loc->type == "Q") {
		...
		if (isEmpty(station) == FALSE) {
			...
			schedule(mkDeparture(leaveTime, departure->locationID), leaveTime);
		}
